  ...
  return (
    <div className="flex flex-col md:flex-row h-[100dvh] bg-robot-dark text-slate-200 font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-h-0 overflow-hidden relative bg-slate-900/50">
         ...
         <div className="md:hidden px-4 py-3 bg-slate-900/95 border-b border-slate-800/50 shrink-0 backdrop-blur-sm z-30">
             <div className="flex bg-slate-800/50 p-1 rounded-xl border border-slate-700/50 relative">
                 {/* Animated Background Pill */}
                 <div 
                    className={`absolute inset-y-1 w-[calc(50%-4px)] bg-gradient-to-r from-redis-600 to-redis-500 rounded-lg shadow-lg shadow-redis-900/20 transition-all duration-300 ease-out ${
                        mobileTab === 'terminal' ? 'left-1' : 'left-[50%]'
                    }`} 
                 />

                 <button 
                    onClick={() => setMobileTab('terminal')}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold transition-colors relative z-10 ${
                        mobileTab === 'terminal' 
                        ? 'text-white' 
                        : 'text-slate-400 hover:text-slate-200'
                    }`}
                 >
                    <TerminalIcon className={`w-4 h-4 ${mobileTab === 'terminal' ? 'stroke-[2.5px]' : ''}`} />
                    <span>Terminal</span>
                 </button>
                 ...
                 >
                    <Database className={`w-4 h-4 ${mobileTab === 'visualizer' ? 'stroke-[2.5px]' : ''}`} />
                    <span>Warehouse</span>
                 </button>
             </div>
         </div>
