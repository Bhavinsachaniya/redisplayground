  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2 text-slate-300">
        <span className="text-redis-500">127.0.0.1:6379&gt;</span>
      ...
      <div className="relative group">
        <motion.div 
          ...
          className={`${entry.status === 'error' ? 'text-red-400' : 'text-green-400'} whitespace-pre-wrap pl-2 border-l-2 border-slate-700/50 pr-8`}
        >
          {entry.output}
        </motion.div>
        ...
        {entry.output && (
          <button
            onClick={handleCopy}
            className="absolute right-0 top-0 p-1.5 rounded-md bg-slate-800 border border-slate-700 text-slate-400 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all duration-200 hover:bg-slate-700 hover:text-white hover:border-slate-600 z-10"
            title="Copy output"
...
const Terminal: React.FC<TerminalProps> = ({ onExecute, history, isActive = true, suggestedCommand }) => {
  ...
  return (
    <div 
      className="flex flex-col h-full bg-black/90 rounded-xl overflow-hidden border border-slate-700 shadow-2xl font-mono text-sm md:text-base relative"
    ...
    >
      ...
      <div className="relative">
        ...
        <AnimatePresence>
            {hint && input.trim() && (
                <motion.div 
                    ...
                    exit={{ opacity: 0 }}
                    className="absolute bottom-full left-4 mb-2 px-3 py-1 bg-slate-800 border border-slate-600 text-xs rounded-lg shadow-xl pointer-events-none flex items-center gap-2"
                >
                    {isMatchingSuggestion ? (
                        <>
                            <span className="text-yellow-400 font-bold">Suggestion:</span>
                        ...
                        <>
                            ...
                            <span className="text-slate-500 font-mono">{hint}</span>
                        </>
                    )}
                </motion.div>
            )}
        ...
        <form onSubmit={handleSubmit} className="p-4 bg-slate-900 border-t border-slate-800 flex items-center gap-2 relative z-10">
            <span className="text-redis-500 font-bold shrink-0">127.0.0.1:6379&gt;</span>
            <input
            ref={inputRef}
            type="text"
            value={input}
            ...
            <button 
                type="submit" 
                className={`p-1.5 rounded-md transition-colors ${input.trim() ? 'bg-redis-600 text-white hover:bg-redis-500' : 'text-slate-600 bg-slate-800 cursor-not-allowed'}`}
                disabled={!input.trim()}
            >
